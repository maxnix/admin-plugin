...
export function skipDescribeIfNetworksIsNot(
  networksToNotSkip: string[],
  reason?: string
) {
  return skipDescribeIf(!networksToNotSkip.includes(hre.network.name), reason);
}
...
export const skipTestSuiteIfNetworkIsNotZkSync = skipDescribeIfNetworksIsNot(
  ZK_SYNC_NETWORKS,
  'ZkSync network'
);
